...
const ENGLISH_SPELL_RULES: EnRule[] = [
  ...
  { wrong: "noticable", correct: ["noticeable"], explanation: "Keeps the 'e' (noticeable)." },
...
export function checkSpelling(text: string, customAcceptedWords?: Set<string>): SpellCheckResult {
  ...
  // Also supports contractions with apostrophes (e.g. don't, it's, they're)
  const wordRegex = /[\p{L}\u0981-\u0983\u09BC\u09BE-\u09CC\u09CD\u09D7\u09DC\u09DD\u09DF\u09CE\u200C\u200D]+(?:['’][\p{L}\u0981-\u0983\u09BC\u09BE-\u09CC\u09CD\u09D7\u09DC\u09DD\u09DF\u09CE\u200C\u200D]+)*/gu;
  ...
  while ((match = wordRegex.exec(text)) !== null) {
    ...
    if (isBengaliWord(rawWord)) {
      ...
      const alreadyValid = isBengaliWordRecognized(rawWord, customAcceptedWords).recognized;
      if (!alreadyValid) {
      ...
      const dialSenMatch = rawWord.match(/^(করসেন|বলসেন|দেখসেন|খাইসেন|পাইসেন|গেসেন|আসসেন|লিখসেন|শুনসেন|জানসেন)$/);
      if (dialSenMatch) {
        const dialWord = dialSenMatch[0];
        const root = dialWord.replace(/সেন$/, "");
        const map: Record<string, string[]> = {
          কর: ["করেছেন", "করছেন"],
          বল: ["বলেছেন", "বলছেন"],
          দেখ: ["দেখেছেন", "দেখছেন"],
      ...
      if (!bnCheck.recognized) {
        const suggestions = bnCheck.suggestions && bnCheck.suggestions.length > 0
          ? bnCheck.suggestions
          : getSuggestions(rawWord, "bn");
        mistakes.push({
          ...
          endIndex,
          suggestions,
          explanation: bnCheck.reason || "শব্দটিতে অশুদ্ধ বর্ণবিন্যাস বা ভুল বানান রয়েছে৤",
          language: "bn",
          category: "spelling",
        });
        bnCount++;
    ...
    if (isEnglishWord(rawWord)) {
      ...
      // Single letter words like 'a' or 'I' are not errors
      if (rawWord.length <= 1) {
        continue;
      }
      ...
      if (!enCheck.recognized) {
        // Capitalized proper nouns (e.g. Dhaka, Bangladesh, Ahona, etc.) are valid names
        if (rawWord[0] === rawWord[0].toUpperCase() && !ENGLISH_MAP.has(lowerEn)) {
          continue;
        }
